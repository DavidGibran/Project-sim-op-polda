        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <form action="{{ route('kendaraan.index') }}" method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center w-full xl:w-auto">
                ...
                <select name="status" onchange="this.form.submit()" class="w-full sm:w-auto rounded-lg border border-gray-300 bg-white py-2 px-4 text-sm text-gray-700 outline-none focus:border-primary focus-visible:shadow-none dark:border-strokedark dark:bg-transparent dark:text-gray-300">
                    <option value="Semua" class="text-gray-700 dark:text-white" {{ request('status') === 'Semua' ? 'selected' : '' }}>Semua Status</option>
                ...
                <a href="{{ route('kendaraan.index') }}" class="text-sm font-medium text-error hover:underline mt-1 sm:mt-0 text-center sm:text-left dark:text-gray-400">Reset Filter</a>
                @endif
            </form>

            <!-- Actions -->
            <div class="flex items-center gap-3 mt-3 xl:mt-0 justify-end">
                <a href="{{ route('kendaraan.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white shadow-theme-sm hover:bg-brand-600 transition-all">
                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
