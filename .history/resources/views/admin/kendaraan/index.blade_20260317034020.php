...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <nav>
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li class="font-medium text-primary dark:text-gray-400">Master Kendaraan</li>
        </ol>
    </nav>
</div>
        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <form action="{{ route('kendaraan.index') }}" method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center w-full xl:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari No. Polisi, Merk, Tipe..." class="w-full rounded-lg border border-gray-200 bg-transparent py-2 pl-4 pr-10 text-sm outline-none focus:border-primary focus-visible:shadow-none dark:border-gray-800 dark:bg-gray-900/50 dark:text-white">
                ...
                <!-- Filter Status -->
                <select name="status" onchange="this.form.submit()" class="w-full sm:w-auto rounded-lg border border-gray-300 bg-white py-2 px-4 text-sm text-gray-700 outline-none focus:border-primary focus-visible:shadow-none dark:border-strokedark dark:bg-transparent dark:text-gray-300">
                ...
                @if(request()->has('search') || request()->has('status'))
                    <a href="{{ route('kendaraan.index') }}" class="text-sm font-medium text-error hover:underline mt-1 sm:mt-0 text-center sm:text-left">Reset Filter</a>
                @endif
            </form>
            
            <!-- Actions -->
            <div class="flex items-center gap-3 mt-3 xl:mt-0 justify-end">
                <a href="{{ route('kendaraan.create') }}" class="inline-flex items-center justify-center dark:text-gray-400 gap-2 rounded-lg bg-primary px-4 py-2 text-center text-sm font-medium text-white hover:bg-primary/90 dark:bg-primary dark:text-white-400 transition-all">
                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-5 py-4">
                        @php
                            $statusConfig = [
                                'Tersedia' => 'bg-success/10 text-success dark:bg-success/20 dark:text-success',
