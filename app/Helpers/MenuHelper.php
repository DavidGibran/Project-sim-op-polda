...
class MenuHelper
{
            ...
            [
                ...
                'subItems' => [
                    [
                        ...
                        'path' => route('laporan.pemakaian', absolute: false),
                    ],
                    [
    ...
    /**
     * Menu utama untuk PENGEMUDI / USER KENDARAAN
     */
    public static function getUserNavItems(): array
    {
        return [
            [
                'icon' => 'dashboard',
                'name' => 'Dashboard',
                'path' => route('kendaraan.dashboard', absolute: false),
            ],
            [
                'icon' => 'car',
                'name' => 'Perjalanan Aktif',
                'path' => route('kendaraan.perjalanan-aktif', absolute: false),
            ],
            [
                'icon' => 'activity',
                'name' => 'Riwayat Pemakaian Saya',
                'path' => route('kendaraan.riwayat-pemakaian', absolute: false),
            ],
        ];
    }

    /**
     * Menentukan menu berdasarkan session/auth yang aktif
     *
     * - Jika kendaraan login -> tampilkan menu user
     * - Jika admin login -> tampilkan menu admin
     */
    public static function getMainNavItems(): array
    {
        if (session()->has('kendaraan_id')) {
            return self::getUserNavItems();
        }

        return self::getAdminNavItems();
    }

    /**
     * Group menu untuk sidebar TailAdmin
     */
    public static function getMenuGroups(): array
    {
        return [
            [
                'title' => session()->has('kendaraan_id') ? 'Menu Pengemudi' : 'Menu Utama',
    ...
    /**
     * Mengecek apakah path sedang aktif
     */
