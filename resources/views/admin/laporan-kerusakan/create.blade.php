...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">Mode Detail: Input lengkap oleh Admin/Teknisi</p>
    </div>
...
<div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
    <form action="{{ route('admin.laporan-kerusakan.store') }}" method="POST" class="p-6 sm:p-8">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="md:col-span-1">
                ...
                <textarea name="detail_teknis" rows="4" placeholder="Jelaskan detail teknis kerusakan jika sudah diketahui..." class="w-full rounded-lg border border-gray-200 bg-transparent py-3 px-4 text-gray-700 outline-none transition focus:border-primary dark:border-gray-800 dark:bg-gray-900 dark:text-white @error('detail_teknis') border-error @enderror">{{ old('detail_teknis') }}</textarea>
                @error('detail_teknis')
                    <p class="mt-1 text-xs text-error">{{ $message }}</p>
                @enderror
            </div>
